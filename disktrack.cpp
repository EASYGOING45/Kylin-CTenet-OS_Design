...
DiskTrack::DiskTrack(QWidget *parent) : QMainWindow(parent)
{
    setWindowTitle(tr("磁盘调度算法衡量模拟"));   //设置标题
    this->setAutoFillBackground(true); // 这句要加上, 否则可能显示不出背景图.
    QPalette palette = this->palette();
    palette.setBrush(QPalette::Window,
       QBrush(QPixmap(":/images/images/back2.jpg").scaled( // 缩放背景图.
       this->size(),
...
//设置 排序GroupBox 可选
void DiskTrack::setorder_on()
{
    orderGroupBox->setEnabled(true);
}

//设置 排序GroupBox 不可选
void DiskTrack::setorder_off(){
    orderGroupBox->setEnabled(false);
}

//按 下一次磁道号 升序排序
void DiskTrack::DISK_AscStablesort(QVector<DISK> *vector){
    std::stable_sort(vector->begin(),vector->end(),[](const DISK &A,const DISK &B){
        return A.Next_number < B.Next_number;});
}

//按 下一次磁道号 降序排序
void DiskTrack::DISK_DescStablesort(QVector<DISK> *vector){
    std::stable_sort(vector->begin(),vector->end(),[](const DISK &A,const DISK &B){
        return A.Next_number > B.Next_number;});
}
